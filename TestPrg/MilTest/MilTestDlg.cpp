...
ON_BN_CLICKED(IDC_LOAD2IMG, &CMilTestDlg::OnBnClickedLoad2img)
ON_BN_CLICKED(IDC_SEL2WND, &CMilTestDlg::OnBnClickedSel2wnd)
ON_BN_CLICKED(IDC_OVLOUT, &CMilTestDlg::OnBnClickedOvlout)
ON_BN_CLICKED(IDC_UNSELWND, &CMilTestDlg::OnBnClickedUnselwnd)
ON_BN_CLICKED(IDC_ADDINTRECT, &CMilTestDlg::OnBnClickedAddintrect)
ON_BN_CLICKED(IDC_BTN_GETOVLDC, &CMilTestDlg::OnBnClickedBtnGetovldc)
ON_BN_CLICKED(IDC_CHKPANZOOM, &CMilTestDlg::OnBnClickedChkpanzoom)
END_MESSAGE_MAP()
...
void CMilTestDlg::OnBnClickedBtnOpen()
{
	...
	m_pImage2 = myMilTest.NewImage();
//	m_pMyDisplay = myMilTest.GetDefaultDisp();
	m_pMyDisplay = myMilTest.NewDisplay();
...
{
	// TODO: Add your control notification handler code here
	m_pImage->SelectWindow(m_pMyDisplay, m_ImageView.m_hWnd);
}
...
//	m_pAddImages = myMilTest.NewImage();
	...
	{
		nameFolder = ImgFolder.GetFolderPath();
		nameFolder+=_T("\\");
...
//	m_pImage->WndLvlTranform(m_pImage, m_pScrnHistoDlg->m_SliderLeft, m_pScrnHistoDlg->m_SliderRight, 0, 4095);
	m_pImage->WndLvlTranform(m_pImage, 4045, 14158, 0, 255);
	m_pImage->Export(_T("d:\\WLT.tif"), M_TIFF);
//	m_pImage->WndLvlTranform(m_pImage, 0, 255, 0, 255);
}
...
void CMilTestDlg::OnBnClickedLoad2img()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	CFileDialog LoadFile(TRUE, _TEXT("*.bmp;*.jpg"));
	if (LoadFile.DoModal())
	{
		m_pImage2->Restore((WCHAR *)(LoadFile.GetPathName().GetString()));
		//		pMyImageProcess->ArithConst(m_pImage, m_pImage, 10, M_SUB_CONST);
	}
...
void CMilTestDlg::OnBnClickedSel2wnd()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	m_pImage2->SelectWindow(m_pMySecondDisplay, &m_imgSpplement);

}


void CMilTestDlg::OnBnClickedOvlout()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	m_pMyDisplay->EnableOverlay();
	m_pMyDisplay->EnableGraph();
	m_pMyDisplay->OutputText(10, 10, _T("This is"));
}


void CMilTestDlg::OnBnClickedUnselwnd()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	m_pImage->SelectWindow(m_pMyDisplay, &m_ImageView, TRUE);
}


void CMilTestDlg::OnBnClickedAddintrect()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	m_pImage->AddInteractiveRect();
}


void CMilTestDlg::OnBnClickedBtnGetovldc()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	m_pMyDisplay->EnableOverlay();
	m_pMyDisplay->EnableGraph();
	CDC *pDC = m_pImage->GetDisplay()->GetDC();
	pDC->Rectangle(CRect(10, 10, 50, 50));
}


void CMilTestDlg::OnBnClickedChkpanzoom()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	m_pMyDisplay->EnableAutoPanZoom();
}
