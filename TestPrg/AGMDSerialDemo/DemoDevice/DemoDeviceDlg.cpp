...
void CDemoDeviceDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_XVALUE, m_iGap1Value);
	DDX_Text(pDX, IDC_YVALUE, m_iGap2Value);
	DDX_Text(pDX, IDC_ZVALUE, m_iGap3Value);
	DDX_Radio(pDX, IDC_RMANUAL, m_iValueMode);
	DDX_Text(pDX, IDC_EDT_USERCALDATE, m_strUserCalDate);
	DDX_Text(pDX, IDC_EDT_CALGAP1, m_iCalGap1);
	DDX_Text(pDX, IDC_EDT_CALGAP2, m_iCalGap2);
	DDX_Text(pDX, IDC_EDT_CALGAP3, m_iCalGap3);
	DDX_Text(pDX, IDC_EDT_JIGGAP, m_uiJigGap);
}
...
BOOL CDemoDeviceDlg::OnInitDialog()
{
	...
	m_cSldXValue.SetRange(0, 65535);
	m_cSldXValue.SetPos(15000);
	m_cSldYValue.SetRange(0, 65535);
	m_cSldYValue.SetPos(15000);
	m_cSldZValue.SetRange(0, 65535);
	m_cSldZValue.SetPos(15000);
	SetCalDefault();
	UpdateData(FALSE);
...
{
	CAGMDPacket::EPacketType eType = m_AGMDPacket.CheckoutPacket();
	...
	{
		m_AGMDPacket.SendSensorInfo(m_cKindOfWafer.GetCurSel() + 1, CT2A(m_strCalDate), CT2A(m_strSerialNo));
		m_bStart = FALSE;
	...
	{
		UpdateData(TRUE);
		m_AGMDPacket.SendMeasureData(m_iGap1Value, m_iGap2Value, m_iGap2Value, m_sTemperature, m_byBattery, m_nPacketNum);
		IncPacketNum();
		m_bStart = TRUE;
		TIMECAPS timecaps;
		timeGetDevCaps(&timecaps, sizeof(TIMECAPS));
		m_wndStart.SetWindowText(_T("시작"));
		m_mmTimerID = timeSetEvent(1000, timecaps.wPeriodMin, MMTimerProc, (DWORD)this, TIME_PERIODIC);//jubatemp
	}
		break;
	...
	{
		UpdateData(TRUE);
		m_AGMDPacket.SendCalInfo(CT2A(m_strUserCalDate), m_uiJigGap, m_iCalGap1, m_iCalGap2, m_iCalGap3);
	}
		break;
	...
	{
		UpdateData(FALSE);
		PC_PACKET_SETCAL* pPacket = (PC_PACKET_SETCAL *)m_AGMDPacket.GetPacket();
		SetUISetCalInfo(pPacket);
	}
		break;
...
LRESULT CDemoDeviceDlg::OnMMTimer(WPARAM wParam, LPARAM lParam)
{
	...
	if (m_serial.IsOpen())
	{
		...
		if (m_bStart)
		{
			...
			m_AGMDPacket.SendMeasureData(m_iGap1Value, m_iGap2Value, m_iGap3Value, m_sTemperature, m_byBattery, m_nPacketNum);
			IncPacketNum();
		}
		CString strState;
...
void CDemoDeviceDlg::OnNMReleasedcaptureSlidXvalue(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	m_iGap1Value = m_cSldXValue.GetPos();
	UpdateData(FALSE);
	*pResult = 0;
}
...
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	m_iGap2Value = m_cSldYValue.GetPos();
	UpdateData(FALSE);
	*pResult = 0;
}
...
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	m_iGap3Value = m_cSldZValue.GetPos();
	UpdateData(FALSE);
	*pResult = 0;
}
...
void CDemoDeviceDlg::OnBnClickedButton()
{
	...
	timeGetDevCaps(&timecaps, sizeof(TIMECAPS));
	m_wndStart.SetWindowText(_T("시작"));
	m_mmTimerID = timeSetEvent(25, timecaps.wPeriodMin, MMTimerProc, (DWORD)this, TIME_PERIODIC);
...
void CDemoDeviceDlg::SetCalDefault()
{
	m_iCalGap1 = -11;
	m_iCalGap2 = -12;
	m_iCalGap3 = -13;
	m_uiJigGap = 15000;
	m_strUserCalDate = _T("170305");

	//SetDlgItemInt(IDC_EDT_CALGAP1, -11);
	//SetDlgItemInt(IDC_EDT_CALGAP2, -12);
	//SetDlgItemInt(IDC_EDT_CALGAP3, -13);
	//SetDlgItemTextW(IDC_EDT_USERCALDATE, _T("170305"));
	//SetDlgItemInt(IDC_EDT_JIGGAP, 15000);
}


void CDemoDeviceDlg::SetCalUI(char * szUserCalDate, int iJigGap, int CalGap1, int CalGap2, int CalGap3)
{
	m_iCalGap1 = CalGap1;
	m_iCalGap2 = CalGap2;
	m_iCalGap3 = CalGap3;
	m_uiJigGap = iJigGap;
	m_strUserCalDate = szUserCalDate;

	SetDlgItemInt(IDC_EDT_CALGAP1, m_iCalGap1);
	SetDlgItemInt(IDC_EDT_CALGAP2, m_iCalGap2);
	SetDlgItemInt(IDC_EDT_CALGAP3, m_iCalGap3);
	SetDlgItemTextW(IDC_EDT_USERCALDATE, m_strUserCalDate);
	SetDlgItemInt(IDC_EDT_JIGGAP, iJigGap);

}


void CDemoDeviceDlg::SetUISetCalInfo(PC_PACKET_SETCAL * pPacket)
{
	SetCalUI(pPacket->chCalDate, pPacket->usCalTargetDist, pPacket->sGap1, pPacket->sGap2, pPacket->sGap3);
}
