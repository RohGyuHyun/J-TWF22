...
void CButtonST::PreSubclassWindow() 
{
	...
	m_nTypeStyle = nBS & BS_TYPEMASK; //! Set initial control type
	...
	if (m_nTypeStyle == BS_OWNERDRAW) {
		AfxMessageBox(_T("Don't use the resource editor \"Owner Draw\" or ModifyStyle(0, BS_OWNERDRAW) before calling PreSubclassWindow"), MB_SYSTEMMODAL | MB_ICONERROR | MB_TOPMOST);
	}

	if ((nBS & BS_CHECKBOX) == BS_CHECKBOX) { //! Check if this is a checkbox
		m_bIsCheckBox = TRUE;
	}
	if ((nBS & BS_AUTORADIOBUTTON) == BS_AUTORADIOBUTTON) { //! Check if this is a radiobox
		m_bIsRadio = TRUE;
	}
	...
	ModifyStyle(BS_TYPEMASK, BS_OWNERDRAW, SWP_FRAMECHANGED); //! Switch to owner-draw
...
UINT CButtonST::OnGetDlgCode() 
{
	...
	nCode |= (m_bIsDefault ? DLGC_DEFPUSHBUTTON : ( m_bIsRadio ? DLGC_RADIOBUTTON : DLGC_UNDEFPUSHBUTTON ) );
...
LRESULT CButtonST::OnSetState(WPARAM wParam, LPARAM lParam)
{
...
void CButtonST::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	::SendMessage(GetParent()->m_hWnd, WM_JOGBTN_LBUTTONDOWN, nFlags, GetDlgCtrlID());
...
{
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.
	::SendMessage(GetParent()->m_hWnd, WM_JOGBTN_LBUTTONUP, nFlags, GetDlgCtrlID());
...
{
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.
	::SendMessage(GetParent()->m_hWnd, WM_JOGBTN_RBUTTONDOWN, nFlags, GetDlgCtrlID());

	CButton::OnRButtonDown(nFlags, point);
}


BOOL IsRadio(HWND hWnd)
{
	unsigned long dlgCode = (unsigned long)::SendMessage(hWnd, WM_GETDLGCODE,0,0L);
	BOOL fIsRadio = (dlgCode & DLGC_RADIOBUTTON) ? TRUE : FALSE;

	return fIsRadio;
}

BOOL IsRadioGroup(HWND hWnd)
{
	unsigned long style = ::GetWindowLong(hWnd, GWL_STYLE);
	BOOL fIsGroup = (style & WS_GROUP) ? TRUE : FALSE;

	return (IsRadio(hWnd) && fIsGroup) ? TRUE : FALSE;
}
...
{	
	...
	if (m_bIsCheckBox)
	{
		...
		//unsigned long state = SendMessage(BM_GETCHECK, 0, 0);
		//BOOL fChecked = (state & BST_CHECKED) ? TRUE : FALSE;

		//if (!GetSelectionState()) {
		//if (fChecked == FALSE) {
		if (GetCheck() == 0) {
			HWND hThisCtrl = GetSafeHwnd(); //! get this button handle
			HWND hParentDlg = GetParent()->GetSafeHwnd(); //! get button parent handle

			if (hThisCtrl && hParentDlg) { //! consistency check
				HWND hStartRadio;
				for (hStartRadio = hThisCtrl; hStartRadio != NULL && IsRadioGroup(hStartRadio) == FALSE; ) {
					HWND hPrev = ::GetNextDlgGroupItem(hParentDlg, hStartRadio, TRUE);
					if (IsRadio(hPrev) == FALSE) {
						break;
					}

					if (hPrev == hStartRadio || hPrev == hThisCtrl) {
				...
				HWND hRadio = hStartRadio;
				for ( ; hRadio != NULL; ) {	//scan the buttons within the group
					if (IsRadio(hRadio)) {
						if (hRadio == hThisCtrl/* || hRadio == NULL*/) {
							::SendMessage(hRadio, BM_SETCHECK, 1, 0L);
							//break;
						}
						else {
							::SendMessage(hRadio, BM_SETCHECK, 0, 0L);
						}
					}

					hRadio = ::GetNextDlgGroupItem(hParentDlg, hRadio, FALSE);
					if (hRadio == hStartRadio || IsRadio(hRadio) == FALSE) {
						break;
					}
				}
			}
...
DWORD CButtonST::SetIcon(HICON hIconIn, HICON hIconOut)
{
	...
	if (hIconIn)
	{
		...
		if (hIconOut)
		{
			if ((HICON)hIconOut == BTNST_AUTO_GRAY)
			{
