...
//	ON_WM_CTLCOLOR_REFLECT()
	ON_WM_CTLCOLOR()
	ON_WM_TIMER()
	ON_NOTIFY_REFLECT(TCN_SELCHANGE, &CXTabCtrl::OnTcnSelchange)
	ON_NOTIFY_REFLECT(TCN_SELCHANGING, &CXTabCtrl::OnTcnSelchanging)
END_MESSAGE_MAP()
...
void CXTabCtrl::OnTcnSelchange(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void CXTabCtrl::OnTcnSelchanging(NMHDR *pNMHDR, LRESULT *pResult)
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
...
//	pDC->FillSolidRect(rect, ::GetSysColor(COLOR_BACKGROUND));
...
int CXTabCtrl::GetYLimit(void)
{
	...
	for (int i=0; i < GetItemCount(); i++)
	{
		GetItemRect(i, WinRect);
		if(YLimit<WinRect.bottom)YLimit=WinRect.bottom;
	}
...
int CXTabCtrl::ApplyYLimit(void)
{
	int YLimit=GetYLimit();
	if(YLimit<0)return -1;
	m_ptTabs.y = YLimit+3;
	return 0;
}

void CXTabCtrl::ReLocatePos(void)
{
	static int YLimit=m_ptTabs.y;
	TCITEM item;
	CWnd* pWnd;
	item.mask = TCIF_PARAM;
...
HBRUSH CXTabCtrl::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
	...
	default:
		return 0;
		//5return CDialog::OnCtlColor(pDC, pWnd, nCtlColor);
	}
