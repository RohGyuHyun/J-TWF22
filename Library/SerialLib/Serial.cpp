...
int CSerial::FindFirstAvailablePort(int nMax)
{
	TCHAR szBuffer[32];

	for (int i = 0; i < nMax; i++)
	{
		_stprintf_s(szBuffer, _T("\\\\.\\COM%d"), i + 1);

		if (CheckPort(szBuffer) == EPortAvailable)
			return i;
	}
...
{	
	...
	{
		// Display a warning
		_RPTF1(_CRT_WARN,"CSerial::Open - Unable to obtain default communication configuration.LE[%d]\n", GetLastError());
	}
