...
{	
		if (!((bool)pFunction))
			CSerialWnd::OnEvent(eEvent, eError);
		else
			pFunction((int)eEvent);
	}

	int RegCallback(tFunction pFunc)
	{
		pFunction = std::move(pFunc);
		return 0;
	}

	int RegAnalizerCallback(tAnalizer pFunc)
	{
		pAnalizer = std::move(pFunc);
		return 0;
	}
};
