...
#include <tchar.h>
#include <mmsystem.h>
#include "SCInspPacket.h"
#pragma comment(lib, "Setupapi")
#pragma comment(lib, "winmm.lib")

IMPLEMENT_DYNAMIC(CSCInspPacket, CObject);

CSCInspPacket::CSCInspPacket(CSerial& serial, LONG lTimeout)
	: m_serial(serial)
	, m_lTimeout(lTimeout)
	, m_bIsMeaInfo(FALSE)
...
CSCInspPacket::~CSCInspPacket()
{
}

BYTE CSCInspPacket::CalcChecksum(const BYTE* pbyData, int nSize)
{
	if (nSize < 2)
		return 0;
...
BYTE CSCInspPacket::DVCProcessComBuffer()
{
...
BYTE CSCInspPacket::ProcessComBuffer()
{
...
CSCInspPacket::EPacketType CSCInspPacket::CheckoutPacket()
{
...
int CSCInspPacket::MakeProcessThread()
{
	m_bTerminateThread = FALSE;
...
UINT CSCInspPacket::ProcessThread(LPVOID pSender)
{
	return 0;
}
...
int CSCInspPacket::GetQueuePacket(BYTE * pPacket, CSCInspPacket::EPacketType tCmdType)
{
	int sizePacket;
...
int CSCInspPacket::LightOnOff(tLightStatus lsStatus, long *pBrightness)
{
	char chCmd;
...
int CSCInspPacket::LightOnOff(int ch, tLightStatus lsStatus)
{
	char chCmd;
	if (lsStatus == LIGHT_ON)
		chCmd = 'o';
...
int CSCInspPacket::SendLightBrightness(int pBrightness[4])
{
	char chCmd='w';
...
int CSCInspPacket::SendLightBrightness(int ch, int iBrightness)
{
	char chCmd = 'w';
...
int CSCInspPacket::SendLightOnOffCMD(int nCh, char chCMD)
{
...
int CSCInspPacket::SendLightCMD(int nCh, char chCMD, int Value)
{
