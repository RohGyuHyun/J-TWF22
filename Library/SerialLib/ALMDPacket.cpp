...
BOOL CALMDPacket::SendMeasureData(WORD wSec, WORD wCount01, WORD wCount03, WORD wCount05,
...
BOOL CALMDPacket::SendStateData(char* pchCalDate, char* pchSerialNo)
{
...
CALMDPacket::EPacketType CALMDPacket::CheckoutPacket()
{
	...
	if (bySTX != 0)
	{
		...
		if (bySTX == STX_SENSOR)
		{
			...
			pPacket->sRoll = (WORD)::ntohs(pPacket->sRoll);
			pPacket->sPitch = (WORD)::ntohs(pPacket->sPitch);
