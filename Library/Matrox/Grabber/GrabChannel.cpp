...
{
	m_pMilBufProc->SetDepth(8+M_UNSIGNED);
//	m_pMilBufDisp->SetDepth()
	m_pMilBufProc->SetAttribute(M_IMAGE + M_DISP + M_PROC + M_DIB);
	m_pMilBufDisp->SetAttribute(M_IMAGE + M_DISP + M_PROC + M_DIB);
	//jubatemp 4/22
//	m_pMilBuffer->Alloc2D(CSize(640,480));
	m_pMilBufProc->Alloc2D(CSize(1000,1000));
...
int CGrabChannel::SetMilBuffer(CMilBuffer * pMilBufProc)
{
	m_pMilBufProc = pMilBufProc;
	return 0;
}

int CGrabChannel::SetMilBuffer(CMilBuffer * pMilBufProc, CMilBuffer * pMilBufDisp)
{
	m_pMilBufProc = pMilBufProc;
