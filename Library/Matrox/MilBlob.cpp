...
{
	return (int)MblobGetNumber(m_milResult, M_NULL);
}
...
{
	return (int)MblobAllocFeatureList(m_pMilBuffer->m_milIDSystem, &m_featureList);
}

int CMilBlob::AllocResult(void)
{
	return (int)MblobAllocResult(m_pMilBuffer->m_milIDSystem, &m_milResult);
}
