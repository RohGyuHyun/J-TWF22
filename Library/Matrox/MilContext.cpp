...
int CMilContext::RestoreContext(TCHAR * szLoadFile)
{
	...
	return (int)MmodRestore(szLoadFile, m_pMilSystem->GetSystemID(), M_DEFAULT, &m_milIDContext);
...
int CMilContext::Alloc(int MFType)
{
	if(MFType==CMilContext::MFT_GEOMETRIC)
		return (int)MmodAlloc(m_pMilSystem->GetSystemID(), M_GEOMETRIC, M_DEFAULT, &m_milIDContext);
	else
		return (int)MmodAlloc(m_pMilSystem->GetSystemID(), M_GEOMETRIC_CONTROLLED, M_DEFAULT, &m_milIDContext);
}
...
void CMilContext::GetImageSize(CSize *pSize)
{
	MIL_INT64 attr = M_NULL;
	MIL_DOUBLE cx=0, cy=0;
//	long cx = 0, cy = 0, Depth = 0, BandSize = 0;
	pSize->cx = MmodInquire(GetIDContext(), 0, M_BOX_SIZE_X, &cx);
	pSize->cy = MmodInquire(GetIDContext(), 0, M_BOX_SIZE_Y, &cy);
}
