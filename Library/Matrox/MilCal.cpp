...
{
	if(m_milIDCal)Free();
	return (int)McalRestore(strFileName.GetBuffer(), CMilSystem::Instance()->GetSystemID(), M_DEFAULT, &m_milIDCal);
}
...
{
	return (int)McalAlloc(CMilSystem::Instance()->GetSystemID(), M_DEFAULT, M_DEFAULT, pMilIDCal);
}
...
{
	if(!m_milIDCal)
		return (int)McalAlloc(CMilSystem::Instance()->GetSystemID(), M_DEFAULT, M_DEFAULT, &m_milIDCal);
	else
		return -1;
}
...
{
	return (int)McalInquire(m_milIDCal, InquireType, pRetValue);
}
