/* (C) 2001-2005 Joaqu? M?L?ez Mu?z (joaquin@tid.es). All rights reserved.
 ...
 *       "Portions copyright 2001 Joaqu? M?L?ez Mu?z (joaquin@tid.es)"
 *
...
static BOOL Win7ListPorts(LISTPORTS_CALLBACK lpCbk, LPVOID lpCbkValue);
    ...
    case VER_PLATFORM_WIN32_NT:
	  ...
	  else
	  {
		  ...
		  case 5:
			  ...
			  break;
		  default:
			  return Win7ListPorts(lpCbk, lpCbkValue); /* Windows 7 and Later OS */
			  break;
		  }
	  }
      break;
...
BOOL Win7ListPorts(LISTPORTS_CALLBACK lpCbk, LPVOID lpCbkValue)
/* 


* HKML\HARDWARE\\DEVICEMAP\\SERIALCOMM
...
*/
{
	...
	for (dwIndex = 0;; ++dwIndex) {
		...
		portinfo.lpPortName = lpPortName;
		portinfo.lpFriendlyName = lpPortName;
		portinfo.lpTechnology = TEXT(""); /* this information is not available */
		if (!lpCbk(lpCbkValue, &portinfo)) {
			goto end; /* listing aborted by callback */
		}
 ...
 */
{
  ...
  for(dwIndex=0;;++dwIndex) {
    ...
    if (dwError=QueryStringValue(hkLevel1,TEXT("PREFIX"),&lpPortName)) {
