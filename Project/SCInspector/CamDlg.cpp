...
BEGIN_MESSAGE_MAP(CCamDlg, CDialogEx)
	...
	ON_WM_CLOSE()
	ON_WM_ERASEBKGND()
	ON_MESSAGE(WM_SENDFN2REMOTE, &CCamDlg::OnSendFN2Remote)
	ON_MESSAGE(WM_SOCK_RECONNECT, &CCamDlg::OnRequstReconnect)
END_MESSAGE_MAP()
...
BOOL CCamDlg::OnInitDialog()
{
	...
	if (!m_pMilCamView)
	{
		...
		m_pMilCamDraw = CRepository::Instance()->m_MilLib.NewImage();
		m_pMilCamProc = CRepository::Instance()->m_MilLib.NewImage();
	}
	if(!CStatus::Instance()->m_bIsOffLine)
	{
		m_pEuresysChannel = CRepository::Instance()->m_EuresysGrabber.NewChannel();
		m_pEuresysChannel->SetMilBuffer(m_pMilCamProc, m_pMilCamDraw);
		m_pEuresysChannel->SetExpTime(CStatus::Instance()->m_lCamExpTime[m_pEuresysChannel->m_nChannel]);
		m_pEuresysChannel->Init();
	...
	m_inspStage = CStatus::Instance()->m_camInspStage[m_iCamBuildNo];
	...
	m_strCamTag = m_strCamName + _T(":Height");
	m_pMilDisp->OutputText(10, 10, m_strCamTag.GetBuffer());


	if (CStatus::Instance()->m_bUseRemoteCamUpdate)
	{
		InitRemoteCam();
	}
...
LRESULT CCamDlg::OnRequstReconnect(WPARAM wparam, LPARAM lparam)
{
	m_pSockContainer->Close();
	m_bRequestReconnect = TRUE;
	return 0;
}

LRESULT CCamDlg::OnSendFN2Remote(WPARAM wparam, LPARAM lparam)
{
	CString strNGImgName = CStatus::Instance()->m_strRemoteImgName;
	SendFN2Remote(strNGImgName);
	return 0;
}

int CCamDlg::InitRemoteCam()
{
	CString strIP = CStatus::Instance()->GetRemoteIPString();
	m_pSockContainer = new CSCSocket;
	m_pSockContainer->Init(this);
	m_pSockContainer->Connect(strIP, BASE_SCREMOTEPORTNO + m_iCamBuildNo);

	return 0;
}

int CCamDlg::ReconnectRemoteCam()
{
	CString strIP = CStatus::Instance()->GetRemoteIPString();
	m_pSockContainer->Close();
	m_pSockContainer->Init(this);
...
int CCamDlg::SendFN2Remote(CString strFileName)
{
	if (CStatus::Instance()->m_nCountOfCamRemote)return 0;
	if (!CStatus::Instance()->m_bUseRemoteCamUpdate)return 0;
	switch (m_iCamBuildNo)
	{
	case 0:
	case 1:
		break;
	...
	CMainFrame* pFrame = (CMainFrame*)AfxGetMainWnd();
	CSCInspectorView *pView = (CSCInspectorView *)pFrame->GetActiveView();
	if (m_bRequestReconnect)
	{
		ReconnectRemoteCam();
		m_bRequestReconnect = FALSE;
	}
	CW2A pszA(strFileName.GetBuffer());
	BOOL bErr;
	if (strlen(pszA) > 29)
	{
		bErr = TRUE;
	...
	int nBytesToSend = strlen(pszA);
	int nBytesSent = m_pSockContainer->Send(pszA, strlen(pszA));
...
int CCamDlg::SaveImage(BOOL bIsNG)
{
	...
	if(bIsNG)PostMessage(WM_SENDFN2REMOTE);
	return 0;
}
...
{
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.
	if (m_pEuresysChannel)m_pEuresysChannel->Close();
