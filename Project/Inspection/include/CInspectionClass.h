...
public:
	...
	void SetBlobsResultDisplay(CMilDisplay* milDisplay, vector<BLOB_RESULT_PARA> stBlobsResult, CListCtrl* list_ctrl = NULL);
	...
	void MBinarizeAdaptive(CMilBuffer* src, CMilBuffer* dst, double dMinContrast = 6.0, double dMinGlobal = 65.0);

	void GetBlobsResult(vector<BLOB_RESULT_PARA>* stBlobsResult);

	void SetInspIndex(int nIdx) { m_nInspIndex = nIdx; };
};
