...
tGetModelClass CModelBase::pGetModelClass = NULL;

// CModelBase
IMPLEMENT_DYNAMIC(CModelBase, CWnd)
...
//		pElement = (*pGetModelClass)(nameElement.GetBuffer());
		pElement = (*pGetModelClass)(nameElement.GetBuffer());
		pElement->SetParentModel(this);
		if(pElement){
...
CModelBase * CModelBase::GetParentModel(void)
{
	return m_pParentModel;
}

void CModelBase::SetParentModel(CModelBase * pParentModel)
{
	m_pParentModel = pParentModel;
}

CString CModelBase::GetParentModelName(void)
{
	USES_CONVERSION;
	if(!GetParentModel())return CString();
	CString nameParent = A2W(GetParentModel()->GetRuntimeClass()->m_lpszClassName);
	return nameParent;
}
