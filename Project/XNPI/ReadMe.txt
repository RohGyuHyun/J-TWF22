...
XFMS.h
    ...
    여기에는 resource.h를 비롯한 다른 프로젝트별 헤더가 포함되어 있으며 CXNPIApp 응용 프로그램 클래스가 선언되어 있습니다.

SCInspector.cpp
    이는 응용 프로그램 클래스 CXNPIApp가 포함된 기본 응용 프로그램 소스 파일입니다.
...
XFMSDoc.h, SCInspectorDoc.cpp - 문서
    이 파일에는 CXNPIDoc 클래스가 포함됩니다. 이 파일을 편집하여 특수 문서 데이터를 추가하고 CXNPIDoc::Serialize를 통해 파일 저장 및 로드를 구현합니다.

XFMSView.h, SCInspectorView.cpp - 문서 뷰
    이 파일에는 CXNPIView 클래스가 포함됩니다.
    CXNPIView 개체는 CXNPIDoc 개체를 보는 데 사용됩니다.
