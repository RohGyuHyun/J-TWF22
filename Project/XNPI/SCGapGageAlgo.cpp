...
int CSCGapGageAlgo::Inspect(CMilBuffer * pMilORG, CMilBuffer * pMilView, CRect * pRectInsp)
{
...
//***************************************************************************
void CSCGapGageAlgo::DrawLegend(MIL_ID MilOverlayImage, bool DrawSubPositions)
{
...
int CSCGapGageAlgo::CheckCriteria()
{
...
void CSCGapGageAlgo::DrawResult(int rv)
{
...
int CSCGapGageAlgo::DrawSubPos(double *pDblXPos, double *pDblYPos, double *pDbl2XPos, double *pDbl2YPos)
{
	for (int i = 0; i < COUNT_SUBREGION; i++)
	{
		if (m_posResult[i]!=-1)
...
int CSCGapGageAlgo::DrawROI(CMilBuffer *pMilView, int iResult)
{
	CRect DoubleRect = m_rectInsp;
	if(!iResult)
...
void CSCGapGageAlgo::GetResult(CSCResult * pResult)
{
