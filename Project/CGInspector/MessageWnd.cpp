...
BOOL CMessageWnd::OnInitDialog()
{
	...
	CRect rectWin(0, 0, GetSystemMetrics(SM_CXFULLSCREEN), GetSystemMetrics(SM_CYFULLSCREEN));
	GetClientRect(&RectStarMessage);
	MoveWindow(rectWin.Width()/2- RectStarMessage.Width()/2, rectWin.Height()/2- RectStarMessage.Height()/2,RectStarMessage.Width(),RectStarMessage.Height(),true);
	SetTimer(0,100,NULL);
