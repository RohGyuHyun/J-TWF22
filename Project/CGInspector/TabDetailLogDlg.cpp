...
BOOL CTabDetailLogDlg::OnInitDialog()
{
	...
	m_ClearBtn.SetIcon((HICON)IDI_CLEAR);
...
void CTabDetailLogDlg::WndRepositioning()
{
	CRect rectMain;
	...
	MoveWindow(0, 30, rectMain.Width()-25, rectMain.Height()-30);
	
	if (!m_LogdataEditControl.m_hWnd)return;
	m_LogdataEditControl.MoveWindow(0, 0, rectMain.Width() - 25, rectMain.Height()-40);

}


void CTabDetailLogDlg::OnSize(UINT nType, int cx, int cy)
{
...
void CTabDetailLogDlg::Puts(CString strText)
{
	SetTextLog(strText, RGB(50, 50, 50), FALSE);
}
