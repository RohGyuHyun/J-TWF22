...
//	MoveWindow(0, 0, rectMain.Width(), WinRect.Height());
...
void CDetailInfoDlg::WndRepositioning()
{
	if (!(m_pTabDetailLogDlg->m_pInstance))return;
	CRect rectMain, rectWnd;
	GetClientRect(rectMain);
	m_TabCtrl.GetWindowRect(&rectWnd);
	m_TabCtrl.MoveWindow(0, 0, rectMain.Width(), rectWnd.Height());
		m_pTabDetailLogDlg->m_pInstance->MoveWindow(50, 0, rectMain.Width() - 10, rectWnd.Height() - 10);
}


void CDetailInfoDlg::OnSize(UINT nType, int cx, int cy)
{
